...
- **Revenue ledger audit — 96.25% self-traffic (2026-07-23).** Task `t_efa5b713` performed on-chain forensic verification of all 26 revenue_ledger rows via `eth_getTransactionReceipt` on Base mainnet, reading each row's USDC Transfer event `from` topic. Result: **23/26 rows ($0.385, 96.25%) are self-traffic** from buyer-wallet (`0xC4852c…`) and PAY_TO (`0x5765ae…`) addresses; 3 rows ($0.015) are genuine external revenue from payer `0x7e571e…`. The `gap.json` reports `revenue_usd=0.4` — the true external revenue is **$0.015**, inflating apparent progress by 26×. Verifiable artifacts: `docs/REVENUE_LEDGER_AUDIT.md` (64 lines), `scripts/verify_revenue_ledger.py` (226 lines, reusable RPC-based ledger auditor). Deliverable lives on branch `wt/t_efa5b713` (commit `b2089a2`); the ledger-audit script produces `~/.hermes/data/x402-data-api-revenue/ledger_verified_summary.json`.
- **First organic dollar: still $0.** The t_efa5b713 forensic audit refined the contamination estimate: 96.25% self-traffic (23/26 rows), leaving only $0.015 in genuine external revenue from a single known payer (`0x7e571e…`). $0.015 is noise, not an organic dollar — the milestone target ($1.00 organic) requires **$0.985 more**, not $0.60.
