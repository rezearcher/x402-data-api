...
let authedFetch: typeof globalThis.fetch = globalThis.fetch;
 ...
 * the parsed response.
 */
async function proxyToWorker(body: unknown): Promise<unknown> {
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  ...
  return data as { tools: Array<{ name: string; description?: string; inputSchema?: unknown }> };
});
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const data = (result as Record<string, unknown>)?.result ?? result;
  return data as { content: Array<{ type: string; text?: string }> };
});
